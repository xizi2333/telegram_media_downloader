...
class FilterTestCase(unittest.TestCase):
    def test_message_date_filter_uses_beijing_time(self):
        download_filter = Filter()
        meta = MetaData()
        message = MockMessage(
            id=1,
            media=False,
            date=datetime(2024, 1, 1, 16, 30, tzinfo=timezone.utc),
            from_user=MockUser(),
        )
        ...
        self.assertEqual(meta.message_date_for_filter, datetime(2024, 1, 2, 0, 30))
        self.assertTrue(
            filter_exec(download_filter, "message_date >= 2024-01-02 00:00:00")
        )
        self.assertTrue(
            filter_exec(download_filter, "message_date < 2024-01-02 01:00:00")
        )
        self.assertFalse(
            filter_exec(download_filter, "message_date < 2024-01-02 00:30:00")
        )
    ...
    def test_string_filter(self):
        ...
        message = MockMessage(
            id=5,
            media=True,
            date=datetime(2022, 8, 5, 14, 35, 12, tzinfo=BEIJING_TIMEZONE),
            chat_title="test2",
            caption=None,
            video=MockVideo(
                mime_type="video/mp4",
        ...
        self.assertEqual(
            meta.message_date,
            datetime(2022, 8, 5, 14, 35, 12, tzinfo=BEIJING_TIMEZONE),
        )
