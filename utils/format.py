...
def to_beijing_datetime(value: Optional[datetime]) -> Optional[datetime]:
    """Convert a datetime to a naive Beijing-time datetime.

    Pyrogram currently creates naive message dates from the local system
    timezone.  Calling ``astimezone`` on a naive value first preserves that
    interpretation, then converts the instant to UTC+8.  The timezone marker
    is removed so the result remains comparable with the filter parser's
    existing naive datetime literals.
    """
    if value is None:
        return None
