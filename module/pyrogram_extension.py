...
def set_meta_data(
    ...
    meta_data.message_date_for_filter = to_beijing_datetime(meta_data.message_date)
    if caption:
...
class HookSession(pyrogram.session.Session):
    ...
    def request_timeout(self: pyrogram.session.Session, request_timeout: int):
        """
        Set the request timeout for the session.

        Args:
            request_timeout (int): The request timeout value in seconds.

        Returns:
            None
        """
        self.WAIT_TIMEOUT = request_timeout
...
class HookClient(pyrogram.Client):
    ...
    def __init__(self, name: str, **kwargs):
        ...
        if "request_timeout" in kwargs:
            value = kwargs.get("request_timeout")
            if value:
    ...
    async def invoke(
        self,
        query,
        retries=pyrogram.session.Session.MAX_RETRIES,
        timeout=None,
        sleep_threshold=None,
        business_connection_id=None,
    ):
        """Invoke a Telegram API request using the configured timeout by default."""
        if timeout is None:
    ...
    async def get_file(
        ...
        """Download file chunks with a bounded no-progress timeout."""
        file_stream = super().get_file(
            file_id,
            file_size,
            limit,
            offset,
        ...
        chunk_timeout = max(request_timeout, 0.1) + self.FILE_CHUNK_TIMEOUT_GRACE

        try:
            while True:
                try:
                    chunk = await asyncio.wait_for(
                        file_stream.__anext__(), timeout=chunk_timeout
                    )
    ...
    async def handle_download(self, packet):
        """Download media chunks while preserving partial files for resume."""
        ...
        if in_memory:
            ...
            if file_size and resume_size > file_size:
                os.remove(partial_path)
                resume_size = 0

            aligned_resume_size = resume_size - resume_size % chunk_size
            if aligned_resume_size != resume_size:
                with open(partial_path, "r+b") as partial_file:
                    partial_file.truncate(aligned_resume_size)
                resume_size = aligned_resume_size

            file = open(partial_path, "ab")
        ...
        try:
            ...
            if isinstance(error, pyrogram.StopTransmission):
                return None

            if isinstance(
                error,
                (pyrogram.errors.FloodWait, pyrogram.errors.FloodPremiumWait),
            ):
                raise

            # Return the partial path so the caller can validate it and retry
            # after refreshing the file reference when necessary.
            return partial_path if not in_memory else None
        else:
            if in_memory:
                file.name = file_name
                return file
    ...
    async def connect(
        ...
        # GetFile may create plain Session instances for CDN downloads, so
        # configure the base class as well as the primary HookSession below.
        pyrogram.session.Session.WAIT_TIMEOUT = self.REQUEST_TIMEOUT
